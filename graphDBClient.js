...
const getAllLocations = async session => {
  return new Promise((resolve, reject) => {
    session
      .run(`MATCH (location) RETURN location`)
      .then(result => {
        ...
        locations = response.map(record =>
          parseLocation(record.get("location"))
        );
  ...
  return new Promise((resolve, reject) => {
    session
      ...
      .then(result => {
        ...
        if (!response) resolve({ message: "Location doesn't exist" });
        ...
        `
            MATCH (location) WHERE id(location) = ${locationId}
                SET 
                    ...
                    ${
                      malePopulationCount
                        ? "location.male = {malePopulationCount}"
                        : ""
                    },
                    ${
                      femalePopulationCount
                        ? "location.female = {femalePopulationCount}"
                        : ""
                    }
            RETURN location
        `,
...
const deleteLocation = (session, locationId) => {
  ...
  return new Promise((resolve, reject) => {
    session
      ...
      .then(() => {
        resolve({ message: "location has been removed" });
      })
      .catch(error => reject(error));
  });
};
...
const deleteAllLocations = session => {
  return new Promise((resolve, reject) => {
    session
      ...
      .then(() => {
        resolve({ message: "All locations have been removed" });
      })
      .catch(error => reject({ error }));
  });
};
